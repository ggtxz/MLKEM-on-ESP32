...
// Número de palavras de estado Keccak
#define PQC_KECCAK_STATE_WORDS        25
#define PQC_KECCAK_INCSTATE_WORDS     26

// Tamanhos em bytes dos contextos
#define PQC_SHAKECTX_BYTES     (sizeof(uint64_t) * PQC_KECCAK_STATE_WORDS)
#define PQC_SHAKEINCCTX_BYTES  (sizeof(uint64_t) * PQC_KECCAK_INCSTATE_WORDS)

// Contexto para SHAKE128 incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_INCSTATE_WORDS];
} shake128incctx;

// Contexto para SHAKE128 não-incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_STATE_WORDS];
} shake128ctx;

// Contexto para SHAKE256 incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_INCSTATE_WORDS];
} shake256incctx;

// Contexto para SHAKE256 não-incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_STATE_WORDS];
} shake256ctx;

// Contexto para SHA3-256 incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_INCSTATE_WORDS];
} sha3_256incctx;

// Contexto para SHA3-384 incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_INCSTATE_WORDS];
} sha3_384incctx;

// Contexto para SHA3-512 incremental
typedef struct {
    uint64_t ctx[PQC_KECCAK_INCSTATE_WORDS];
} sha3_512incctx;
